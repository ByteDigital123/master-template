...
class EloquentCourseRepository extends BaseRepository implements CourseInterface
{
    ...
    public function create(array $attributes)
    {
        ...
        $provider_price = explode(".", floatval($attributes['provider_price'] * 100))[0];
        $retail_price = explode(".", floatval($attributes['retail_price'] * 100))[0];

        if(isset($attributes['discounted_retail_price'])){
            $discounted_retail_price = explode(".", floatval($attributes['discounted_retail_price'] * 100))[0];
        }
